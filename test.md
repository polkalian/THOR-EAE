...
A6. We reply to the questions as follows. First, counterfactual policy boosts the exploration of new strategies by accumulating new experiences posterior to that from the original policy. During RL training, an experience is commonly denoted as a tuple $e_t=\left \langle s_t,a_t,r_t,s_{t+1} \right \rangle \in E$, where $r_t,s_{t+1}$ are determined by $s_t$ and $a_t$. Since the counterfactual policy takes its intent into consideration that $s_t^{\prime}=(s_t,i_t)$, experiences from it $E^{\prime}$ include more information than experiences from a standard policy $E$. Moreover, the action distribution follows $P(a_t^{\prime}|i_t)=\pi(s_t,i_t)$, which is posterior to the intent distribution $P(i_t)=\pi(s_t)$. Thus $E^{\prime}$ can be divided into two sets: experimental ones $E_{exp}|\_{a_t=i_t}$ and counterfactual ones $E_{ctf}|_{a_t \neq i_t}$.
