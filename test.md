...
* First, counterfactual policy boosts the exploration of new strategies by accumulating new experiences posterior to that from the original policy. During RL training, an experience used for loss calculation is commonly denoted as a tuple $e_t=\left \langle s_t,a_t,r_t,s_{t+1} \right \rangle \in E$, where $r_t,s_{t+1}$ are determined by $s_t$ and $a_t$. Since the counterfactual policy takes its intent into consideration that $s_t^{\prime}=(s_t,i_t)$, experiences from it $E^{\prime}$ include more information than experiences from a standard policy $E$. Moreover, the action distribution follows $P(a_t|i_t)=\pi(s_t,i_t)$, which is posterior to the intent distribution $P(i_t)=\pi(s_t)$. Thus $E^{\prime}$ can be divided into two sets: experimental ones $E_{exp}|\_{a_t=i_t}$ that the original policy "would have collected", and counterfactual ones $E_{ctf}|\_{a_t \neq i_t}$ that are new experiences exploring other strategies. 

* Therefore, second, there are three advantanges of counterfactual exploration compared with stochastic exploration:1) $s_t^{\prime}$ provides more knowledge about the environment (discussed below). 2) While stochastic experiences are independently distributed, $E_{ctf}$ are collected based on the original strategy, which are more useful. For example, an agent, who try to push a box knowing its original intent is to bypass it, may learn that pushing is more effective than bypassing in this situation since it receives a high reward. Yet random trials can be hard to correlate and result in inefficient learning. 3) Stochastic experiences are not applicable to on-policy algorithm like PPO since random policy differs too much from the policy we are training. In contrast, counterfactual policy explore new experiences by merely change the input (add intent) while maintains the same policy function $\pi$.
...
* We report the performances of models (in Table 3,4) by averaging their scores on 5 tests with the same random seed. We now evaluate several methods on multiple seeds in the table below (in the form of *mean* $pm$ *variance*).
...
* We argue that it is hard to determine which obstacle should be moved more than others, even if it lies along the shortest path. For example, a large-size table lies in front of the agent on the shortest path, with an apple on the right side of the table. Obviously, moving the table aside is harder than picking up the apple and may be less efficient, which makes the table less "relevant". However, we agree there should be an analysis on how well the models perform interactions. Thus we evaluate two additional interaction metrics in the table below to measure the interactive ability:1) Interaction Success Rate (ISR): the ratio of taking interactive actions successfully. Situations such as no obstacle around, obstacle getting stuck, or interaction mismatch may result in failed interactions. 2) Interaction Reward (IR): the accumulated reward of interactions in an episode (i.e. the reduction of geodesic distance $\Delta_{dis}$ caused by interactions).


**References**

[1] Pearl, Judea. "The do-calculus revisited." Proceedings of the Twenty-Eighth Conference on Uncertainty in Artificial Intelligence. 2012.
